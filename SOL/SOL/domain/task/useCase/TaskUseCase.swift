...
public class TaskUseCase: UseCase<TaskEntity, Bool>{
    ...
    override public func execute(_ callback: @escaping (TaskEntity?, Bool) -> Void) {
        port.findById(input.taskId, responseFunc: { (success:BaseApiResponse<TaskResponse>?, error:BaseApiErrorResponse?, isSuccess:Bool) in
